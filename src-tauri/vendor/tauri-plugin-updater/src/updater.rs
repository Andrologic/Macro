...
impl Updater {
    pub async fn check(&self) -> Result<Option<Update>> {
        ...
        if should_update {
            let mut update = self.update_from_release(release)?;
            update.raw_json = raw_json.unwrap();
            Ok(Some(update))
        } else {
            Ok(None)
        }
    ...
    /// Reconstructs an update from persisted release metadata without network access.
    /// Callers must verify cached package bytes before calling `Update::install`.
    pub fn update_from_release(&self, release: RemoteRelease) -> Result<Update> {
        let target = self
            .target
            .as_deref()
            .unwrap_or(updater_os().ok_or(Error::UnsupportedOs)?);
        let installer = installer_for_bundle_type(bundle_type());
...
mod tests {
    #[test]
    fn cached_release_reconstruction_never_discovers_the_channel() {
        let updater = super::UpdaterBuilder {
            run_on_main_thread: Box::new(|_| panic!("no native installation in this test")),
            app_name: "Synthetic".into(),
            current_version: "1.0.0".parse().unwrap(),
            config: crate::Config::default(),
            version_comparator: Some(std::sync::Arc::new(|_, _| panic!("no channel comparison"))),
            executable_path: Some(std::path::PathBuf::from(
                "/tmp/Synthetic.app/Contents/MacOS/Synthetic",
            )),
            target: Some("stable-darwin-aarch64".into()),
            endpoints: Some(vec!["https://offline.invalid/channel.json"
                .parse()
                .unwrap()]),
            headers: Default::default(),
            timeout: None,
            ...
            current_exe_args: Vec::new(),
            on_before_exit: None,
            configure_client: Some(std::sync::Arc::new(|_| {
                panic!("no HTTP client may be built")
            })),
        }
        .build()
        .unwrap();
        let release = serde_json::from_value(serde_json::json!({
            "version": "1.1.0", "notes": "cached", "url": "https://cached.invalid/unused",
            "signature": "synthetic-signature"
        }))
        .unwrap();
        let update = updater.update_from_release(release).unwrap();
        assert_eq!(update.version, "1.1.0");
        assert_eq!(update.signature, "synthetic-signature");
        assert_eq!(update.current_version, "1.0.0");
        assert_eq!(update.target, "stable-darwin-aarch64");
    }
