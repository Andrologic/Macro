use super::store::{
    ...
    parse_chunked_manifest, parse_copilot_secret, read_chunked_secret, split_by_storage_limit,
    storage_limit_bytes, storage_size_bytes, ChunkedSecretPayload, KeyringSecretStore,
    SecretEnvelope, SecretStore,
};
use super::{CopilotSecret, SecretError};
use tracing::{debug, info, warn};

pub fn get_copilot_secret(provider_id: &str) -> Result<Option<CopilotSecret>, SecretError> {
    let store = KeyringSecretStore;
    let Some(secret) = store.read_entry(provider_id)? else {
        return Ok(None);
    };

    if let Some(parsed) = parse_copilot_secret(&secret)? {
    ...
    if let Some(manifest) = parse_chunked_manifest(&secret) {
        ...
        return parse_copilot_secret(&serialized);
    }

    Ok(None)
}

pub fn set_copilot_secret(provider_id: &str, secret: &CopilotSecret) -> Result<(), SecretError> {
    let store = KeyringSecretStore;
    let profile = active_storage_profile();
    let envelope = SecretEnvelope {
        version: 1,
        kind: "copilot_session".to_string(),
    ...
    debug!(
        ...
        storage_limit,
        profile = ?profile,
        "prepared Copilot secret for keyring persistence"
    );
    ...
    info!(
        ...
        "chunking Copilot secret for keyring persistence"
    );
    ...
    for (entry_id, chunk) in chunk_entry_ids.iter().zip(chunks.iter()) {
        if let Err(error) = store.write_entry(entry_id, chunk) {
            warn!(
                ...
                entry_id = %entry_id,
                error = %error,
                "failed to write Copilot secret chunk"
            );
    ...
    let manifest = SecretEnvelope {
        version: 1,
        kind: "copilot_session_chunked".to_string(),
...
pub fn delete_copilot_secret(provider_id: &str) -> Result<(), SecretError> {
    let store = KeyringSecretStore;
