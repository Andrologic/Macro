...
mod tests {
    ...
    fn test_debug_reset_leaves_linked_and_external_worktrees_untouched() {
        ...
        let error = GitState::new()
            .debug_reset_macro_project_artifacts(temp.path())
            .expect_err("linked roots must be refused before reset");
    ...
    #[test]
    fn test_ensure_branch_worktree_preserves_existing_worktree_when_gitfile_is_missing() {
        let temp = TempDir::new().expect("temp dir");
        let repo = init_repo(temp.path());
        let state = GitState::new();
        ...
        state
            ...
            .expect_err("repair must preserve the original branch and files");
        assert_eq!(fs::read_to_string(&preserved_file).unwrap(), "preserve me");
        assert!(repo
            .find_worktree("macro-integration-integration-web-plan-missing-gitfile")
            .is_ok());
    }
