...
fn expand_paths(repo_root: &Path, input: &str) -> Result<Vec<PathBuf>> {
	...
	if is_glob_pattern(input) {
		...
		for entry in glob::glob(absolute.to_string_lossy().as_ref()).map_err(|e| BackendError::Git {
			message: e.to_string(),
		})? {
...
pub fn build_git_log(repo: &Repository, limit: usize, branch: Option<&str>) -> Result<Vec<GitCommitDto>> {
	...
	for commit in commits.iter_mut() {
		...
		if let Some(parent) = commit.parent_ids.first() {
			...
			depth = if *seen == 0 { base_depth } else { next_depth + 1 };
			*seen += 1;
		}
		if depth > next_depth {
			next_depth = depth;
...
pub(crate) fn build_git_branches(repo: &Repository) -> Result<GitBranchesDto> {
	...
	for branch in repo.branches(Some(BranchType::Local))? {
		...
		let commit = branch
			...
			.unwrap_or_default();
		local.push(GitBranch { name, is_head, commit });
	}
