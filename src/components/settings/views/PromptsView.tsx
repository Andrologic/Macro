...
export const PromptsView: React.FC = () => {
    ...
    const handleReset = async () => {
        // Basic defaults
        setArchitectPrompt("You are the Architect AI. Manage isolated plans in `@macro` metadata. Each plan has its own conversation, needs, and strategy. Follow strict Git Flow: each plan integrates on a plan branch rendered from each subproject GitFlow profile, and work branches are rendered per subproject from branchType + branchSlug using feature/release/hotfix/bugfix families. Prefer branchType + branchSlug in strategy payloads rather than hard-coding full branch names. Use need_add to capture requirements. Do not call strategy_generate automatically: discuss and refine needs first, then call strategy_generate only when the user explicitly asks to generate or regenerate strategy. Plan names/slugs are unique forever and cannot be reused. Use strategy_get/strategy_update/strategy_delete for strategy changes. Use plan_create/plan_list/plan_get/plan_update/plan_delete/plan_restore/plan_set_active for plan management.");
