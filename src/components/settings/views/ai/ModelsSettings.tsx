...
export const ModelsSettings: React.FC = () => {
  ...
  // Only show providers that are either ChatGPT (linked somehow) or enabled normal providers. 
  // You might want to see all of them, but empty disabled ones don't add much value. Let's show all for now.
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                        <div className="font-medium text-sm flex items-center gap-2">
                           {provider.name}
                           {!provider.isEnabled && <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded text-muted-foreground">Disabled</span>}
                        </div>
              ...
              <AccordionContent className="pt-0 pb-4">
                <div className="border-t border-border pt-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {provider.providerType === 'openrouter' && (
                        <div className="flex items-center gap-2 px-3 py-1.5 bg-muted/30 rounded-md border border-border/50">
                          <span className="text-xs text-muted-foreground font-medium">Free only</span>
                          <Switch
                            className="scale-90"
                  ...
                  <div className="space-y-2 max-h-[320px] overflow-y-auto pr-2 custom-scrollbar">
                    {filteredModels.map((model) => (
                      <div key={model.id} className="flex items-center justify-between p-3 rounded-md border border-border/60 bg-muted/20 hover:bg-muted/40 transition-colors">
                        <div>
                          <div className="flex items-center gap-2">
                          ...
                          <div className="text-xs text-muted-foreground mt-0.5 font-mono">{model.id}</div>
                        </div>
                        <Switch
      ...
      {addingModelForProvider && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl border border-border bg-card p-5 shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-base font-semibold">Add Custom Model</h4>
              <button
