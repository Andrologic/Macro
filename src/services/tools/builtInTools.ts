import type { Tool, MCPServer } from '../../types';

// Internal Tools - Only real, functional tools
export const BUILT_IN_TOOLS: Tool[] = [
  // Web Search Tool (uses Tavily/Brave API)
  {
    id: 'web_search',
    name: 'Web Search',
    category: 'web',
    status: 'enabled',
    description: 'Search the web for information with citations',
    icon: 'search',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: false,
      provider: 'tavily',
      maxResults: 5,
    },
  },
  // Web fetch tool (direct URL retrieval, no API key required)
  {
    id: 'web_fetch',
    name: 'Web Fetch',
    category: 'web',
    status: 'enabled',
    description: 'Fetch and read a specific URL directly',
    icon: 'globe',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: false,
    },
  },
  {
    id: 'question',
    name: 'Question',
    category: 'ai',
    status: 'enabled',
    description: 'Ask the user a structured blocking question with guided answers',
    icon: 'message-circle-question',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
      locked: false,
    },
  },
  // File reading tool
  {
    id: 'read_file',
    name: 'Read File',
    category: 'filesystem',
    status: 'enabled',
    description: 'Read attached files and context documents in this chat',
    icon: 'file-text',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: false,
    },
  },
  {
    id: 'skill_activate',
    name: 'Activate Skill',
    category: 'ai',
    status: 'enabled',
    description: 'Load the full instructions for an enabled Macro skill',
    icon: 'sparkles',
    config: {
      enabled: true,
      visible: false,
      chatMode: true,
      chatToolboxVisible: false,
      internal: true,
      locked: true,
    },
  },
  {
    id: 'skill_read_resource',
    name: 'Read Skill Resource',
    category: 'ai',
    status: 'enabled',
    description: 'Read a bundled reference or asset from an enabled Macro skill',
    icon: 'book-open',
    config: {
      enabled: true,
      visible: false,
      chatMode: true,
      chatToolboxVisible: false,
      internal: true,
      locked: true,
    },
  },
  {
    id: 'skill_run_script',
    name: 'Run Skill Script',
    category: 'ai',
    status: 'enabled',
    description: 'Run a trusted script bundled with an enabled Macro skill',
    icon: 'play',
    config: {
      enabled: true,
      visible: false,
      chatMode: true,
      chatToolboxVisible: false,
      internal: true,
      locked: true,
    },
  },
  {
    id: 'list',
    name: 'List Files',
    category: 'filesystem',
    status: 'enabled',
    description: 'List files and directories in the workspace',
    icon: 'folder',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: false,
    },
  },
  {
    id: 'read',
    name: 'Read Workspace File',
    category: 'filesystem',
    status: 'enabled',
    description: 'Read files directly from the workspace by path',
    icon: 'file-text',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: false,
    },
  },
  {
    id: 'write',
    name: 'Write Workspace File',
    category: 'filesystem',
    status: 'enabled',
    description: 'Create or overwrite workspace files',
    icon: 'save',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: false,
    },
  },
  {
    id: 'edit',
    name: 'Edit Workspace File',
    category: 'filesystem',
    status: 'enabled',
    description: 'Edit workspace files using exact text replacements',
    icon: 'edit',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: false,
    },
  },
  {
    id: 'delete',
    name: 'Delete Workspace File',
    category: 'filesystem',
    status: 'enabled',
    description: 'Delete a workspace file',
    icon: 'trash',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: false,
    },
  },
  {
    id: 'glob',
    name: 'Glob Files',
    category: 'filesystem',
    status: 'enabled',
    description: 'Find files by glob pattern in the workspace',
    icon: 'search',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: false,
    },
  },
  {
    id: 'grep',
    name: 'Grep Workspace',
    category: 'filesystem',
    status: 'enabled',
    description: 'Search file content in the workspace',
    icon: 'search',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: false,
    },
  },
  {
    id: 'git_status',
    name: 'Git Status',
    category: 'git',
    status: 'enabled',
    description: 'Get repository status',
    icon: 'git-branch',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'git_log',
    name: 'Git Log',
    category: 'git',
    status: 'enabled',
    description: 'List repository commits',
    icon: 'history',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'git_branch_list',
    name: 'Git Branches',
    category: 'git',
    status: 'enabled',
    description: 'List local and remote branches',
    icon: 'git-branch',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'git_diff',
    name: 'Git Diff',
    category: 'git',
    status: 'enabled',
    description: 'Generate git diff',
    icon: 'split',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'git_get_tree',
    name: 'Git Tree',
    category: 'git',
    status: 'enabled',
    description: 'Get predicted git tree',
    icon: 'folder-tree',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'git_add',
    name: 'Git Add',
    category: 'git',
    status: 'enabled',
    description: 'Stage files in git index',
    icon: 'plus',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'git_commit',
    name: 'Git Commit',
    category: 'git',
    status: 'enabled',
    description: 'Create commit in repository',
    icon: 'check',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'git_checkout',
    name: 'Git Checkout',
    category: 'git',
    status: 'enabled',
    description: 'Checkout branch or commit',
    icon: 'git-branch',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'git_merge',
    name: 'Git Merge',
    category: 'git',
    status: 'enabled',
    description: 'Merge a source branch into a target branch',
    icon: 'git-merge',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'git_reset',
    name: 'Git Reset',
    category: 'git',
    status: 'enabled',
    description: 'Reset repository state',
    icon: 'undo-2',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'git_stash',
    name: 'Git Stash',
    category: 'git',
    status: 'enabled',
    description: 'Stash local changes',
    icon: 'archive',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'terminal_create_session',
    name: 'Terminal Session',
    category: 'terminal',
    status: 'enabled',
    description: 'Create a terminal session bound to one subproject',
    icon: 'terminal',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'terminal_run',
    name: 'Terminal Run',
    category: 'terminal',
    status: 'enabled',
    description: 'Run a command in an existing terminal session',
    icon: 'terminal',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'terminal_read',
    name: 'Terminal Read',
    category: 'terminal',
    status: 'enabled',
    description: 'Read the latest output from a terminal session',
    icon: 'terminal',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  {
    id: 'terminal_kill',
    name: 'Terminal Kill',
    category: 'terminal',
    status: 'enabled',
    description: 'Stop the active process in a terminal session',
    icon: 'terminal',
    config: {
      enabled: true,
      visible: true,
      chatMode: false,
      internal: false,
    },
  },
  // Internal source citation helper (always enabled, visible in chat toolbox)
  {
    id: 'mark_source_passage',
    name: 'Sources',
    category: 'ai',
    status: 'enabled',
    description: 'Track interesting passages and passages used by the AI',
    icon: 'book-open',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      chatToggleGroup: 'sources',
      internal: true,
    },
  },
  {
    id: 'read_sources',
    name: 'Read Sources',
    category: 'ai',
    status: 'enabled',
    description: 'Read and filter source passages already saved in this conversation',
    icon: 'book-open',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      chatToolboxVisible: false,
      chatToggleGroup: 'sources',
      internal: true,
    },
  },
  {
    id: 'edit_source_passage',
    name: 'Edit Sources',
    category: 'ai',
    status: 'enabled',
    description: 'Update, reclassify, or delete saved source passages',
    icon: 'edit',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      chatToolboxVisible: false,
      chatToggleGroup: 'sources',
      internal: true,
    },
  },
  {
    id: 'need_add',
    name: 'Add Need',
    category: 'productivity',
    status: 'enabled',
    description: 'Add a structured need to the active Architect plan',
    icon: 'plus',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'need_list',
    name: 'List Needs',
    category: 'productivity',
    status: 'enabled',
    description: 'List and lightly filter needs on the active Architect plan',
    icon: 'list',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'need_get',
    name: 'Get Need',
    category: 'productivity',
    status: 'enabled',
    description: 'Read one need from the active Architect plan',
    icon: 'file-text',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'need_update',
    name: 'Update Need',
    category: 'productivity',
    status: 'enabled',
    description: 'Patch one need on the active Architect plan',
    icon: 'edit',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'need_delete',
    name: 'Delete Need',
    category: 'productivity',
    status: 'enabled',
    description: 'Delete one need from the active Architect plan with confirmation',
    icon: 'trash',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'strategy_generate',
    name: 'Generate Strategy',
    category: 'productivity',
    status: 'enabled',
    description: 'Generate strategy from collected needs for the active plan using identifier-based plan branches',
    icon: 'git-merge',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'task_todo_get',
    name: 'Read Task Todos',
    category: 'productivity',
    status: 'enabled',
    description: 'Read the checklist for the current Architect implementation task',
    icon: 'list-todo',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'task_todo_update',
    name: 'Update Task Todos',
    category: 'productivity',
    status: 'enabled',
    description: 'Update the checklist for the current Architect implementation task',
    icon: 'list-todo',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'task_artifact_list',
    name: 'List Task Artifacts',
    category: 'productivity',
    status: 'enabled',
    description: 'List durable artifacts visible to the current Architect implementation task',
    icon: 'file-text',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'task_artifact_get',
    name: 'Read Task Artifact',
    category: 'productivity',
    status: 'enabled',
    description: 'Read the full content of a visible task artifact',
    icon: 'file-text',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'task_artifact_put',
    name: 'Store Task Artifact',
    category: 'productivity',
    status: 'enabled',
    description: 'Create or replace a durable artifact for the current Architect implementation task',
    icon: 'file-text',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'plan_create',
    name: 'Create Plan',
    category: 'productivity',
    status: 'enabled',
    description: 'Create a new Architect plan with a generated identifier and optional secondary label',
    icon: 'plus-square',
    config: {
      enabled: true,
      visible: false,
      chatMode: false,
      internal: true,
    },
  },
  {
    id: 'plan_list',
    name: 'List Plans',
    category: 'productivity',
    status: 'enabled',
    description: 'List Architect plans for a target code branch',
    icon: 'list',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'plan_get',
    name: 'Get Plan',
    category: 'productivity',
    status: 'enabled',
    description: 'Read a specific Architect plan',
    icon: 'file-text',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'plan_update',
    name: 'Update Plan',
    category: 'productivity',
    status: 'enabled',
    description: 'Update the optional label, description, or mutable slug of an Architect plan',
    icon: 'edit',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'plan_delete',
    name: 'Delete Plan',
    category: 'productivity',
    status: 'enabled',
    description: 'Soft-delete or purge an Architect plan',
    icon: 'trash',
    config: {
      enabled: true,
      visible: false,
      chatMode: false,
      internal: true,
    },
  },
  {
    id: 'plan_restore',
    name: 'Restore Plan',
    category: 'productivity',
    status: 'enabled',
    description: 'Restore a previously deleted Architect plan',
    icon: 'rotate-ccw',
    config: {
      enabled: true,
      visible: false,
      chatMode: false,
      internal: true,
    },
  },
  {
    id: 'plan_set_active',
    name: 'Set Active Plan',
    category: 'productivity',
    status: 'enabled',
    description: 'Set which Architect plan is active for a code branch',
    icon: 'target',
    config: {
      enabled: true,
      visible: false,
      chatMode: false,
      internal: true,
    },
  },
  {
    id: 'strategy_get',
    name: 'Get Strategy',
    category: 'productivity',
    status: 'enabled',
    description: 'Read strategy nodes and branches for the active Architect plan',
    icon: 'git-merge',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'strategy_update',
    name: 'Update Strategy',
    category: 'productivity',
    status: 'enabled',
    description: 'Modify or replace strategy nodes for the active Architect plan',
    icon: 'edit',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
  {
    id: 'strategy_delete',
    name: 'Delete Strategy',
    category: 'productivity',
    status: 'enabled',
    description: 'Delete strategy for the active Architect plan with confirmation',
    icon: 'trash',
    config: {
      enabled: true,
      visible: true,
      chatMode: true,
      internal: true,
    },
  },
];

// MCP Servers - Empty by default (users can configure their own)
export const BUILT_IN_MCP_SERVERS: MCPServer[] = [];

// Default settings
export const defaultToolSettings: Record<string, boolean> = {
  web_search: true,
  web_fetch: true,
  read_file: true,
  skill_activate: true,
  skill_read_resource: true,
  skill_run_script: true,
  list: true,
  read: true,
  write: true,
  edit: true,
  delete: true,
  glob: true,
  grep: true,
  git_status: true,
  git_log: true,
  git_branch_list: true,
  git_diff: true,
  git_get_tree: true,
  git_add: true,
  git_commit: true,
  git_checkout: true,
  git_merge: true,
  git_reset: true,
  git_stash: true,
  terminal_create_session: true,
  terminal_run: true,
  terminal_read: true,
  terminal_kill: true,
  mark_source_passage: true,
  read_sources: true,
  edit_source_passage: true,
  need_add: true,
  need_list: true,
  need_get: true,
  need_update: true,
  need_delete: true,
  strategy_generate: true,
  task_todo_get: true,
  task_todo_update: true,
  task_artifact_list: true,
  task_artifact_get: true,
  task_artifact_put: true,
  plan_create: true,
  plan_list: true,
  plan_get: true,
  plan_update: true,
  plan_delete: true,
  plan_restore: true,
  plan_set_active: true,
  strategy_get: true,
  strategy_update: true,
  strategy_delete: true,
};
export const defaultMCPServerSettings: Record<string, boolean> = {};
