...
| C6. Les chemins restent contenus. | Placer un lien symbolique, une jonction Windows ou un point de réanalyse dans un parent, puis lire ou restaurer un enfant. | TypeScript ne construit pas de cible externe et Rust refuse l'accès par capacité. Aucun octet externe n'est lu, écrit, renommé ou supprimé. | Scénarios Unix et Windows avec fichier sentinelle extérieur. |
...
| C8. Les lectures et l'hydratation sont bornées. | Utiliser un blob, un lien ou un fichier de plusieurs fois la limite, ou un très grand nombre de chemins. | Macro lit au plus la limite plus un octet, borne le nombre de cibles et abandonne avant mutation si la limite cumulée est dépassée. | Lecteurs instrumentés, fichiers creux et compteurs de chemins. |
| C9. Un rollback ne retire que les effets de l'opération. | Faire échouer la deuxième entrée d'une restauration, puis remplacer la première entrée publiée par une écriture concurrente. | Le rollback conserve l'écriture concurrente et retire seulement les temporaires ou parents dont l'empreinte correspond encore à celle de l'opération. | Interleaving déterministe avec échec partiel et empreinte de publication. |
