...
Une mutation qui repose sur une lecture préparatoire doit réutiliser la révision observée lorsque l'agent n'en fournit pas. La relecture de validation et l'enregistrement du checkpoint font partie de la même opération récupérable : leur échec restaure toutes les cibles encore restaurables sans écraser une modification externe concurrente. Un noyau distant doit annoncer `recoverable_checkpoints_v1` et produire les instantanés avant/après nécessaires lorsqu'un checkpoint est attendu ; sinon, il doit refuser la mutation. Il doit aussi annoncer `idempotent_tool_execution_v1`, lier chaque mutation à un `execution_id` non réutilisable pour une autre requête, achever l'opération indépendamment de la connexion HTTP et rejouer son résultat durable si la réponse initiale est perdue. Ces instantanés restent des données internes de récupération et ne peuvent pas être demandés au moyen des arguments d'un outil agent.
