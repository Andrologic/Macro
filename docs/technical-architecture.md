...
`grep` ignore les fichiers binaires et les fichiers de plus de 4 Mio, puis rend ces omissions visibles dans `skipped_files`. Le pont Copilot envoie `list`, `read`, `glob`, `grep` et `ast_grep` au tool host natif : ces appels partagent donc le confinement, les exclusions, les échéances et les bornes du backend. `read_file` reste relayé au frontend afin de pouvoir relire aussi les pièces jointes et les sorties `tool-output://` avec leurs arguments de pagination brute.
