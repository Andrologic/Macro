...
Dans un processus Macro, chaque mutation acquiert un verrou associé à la cible canonique avant de valider la révision et le conserve jusqu'à la fin de l'écriture, de la suppression ou du rollback. Les lots multi-fichiers trient et dédupliquent leurs verrous avant acquisition afin d'éviter les interblocages. Les écritures utilisent ensuite un fichier temporaire et un renommage atomique, avec une dernière revalidation au plus près du renommage. Cette exclusion ne s'étend pas aux applications externes qui n'utilisent pas le protocole : elles peuvent encore gagner une course dans l'intervalle résiduel. Lorsqu'un agent omet la révision pour `edit`, `delete` ou une mise à jour/suppression par patch, le fallback frontend réutilise automatiquement la révision observée pendant la préparation afin de conserver la protection optimiste. Un transport distant doit annoncer `content_revisions_v1` avant d'accepter l'une de ces mutations, même si l'appel n'indique pas explicitement la révision. Lorsqu'un callback de checkpoint est requis, il doit aussi annoncer `recoverable_checkpoints_v1` : le noyau capture alors les instantanés avant/après sous les mêmes verrous que la mutation et ne rend le succès qu'avec un checkpoint complet. Le noyau headless annonce en plus `idempotent_tool_execution_v1` : chaque mutation exige un `execution_id`, s'exécute dans une tâche détachée du cycle de vie HTTP, puis son résultat et son checkpoint sont journalisés atomiquement dans le dossier de configuration. Une nouvelle requête strictement identique avec le même identifiant attend l'exécution en cours ou relit le résultat journalisé ; une empreinte différente est refusée. Le frontend ne retente qu'une erreur de transport et réutilise exactement le même corps. Si la persistance du checkpoint côté client échoue, Macro restaure chaque cible en ordre inverse avec la révision après mutation comme garde. La relecture nécessaire aux replays passe par la route headless authentifiée `/tools/checkpoint-snapshot`; elle active une option Rust interne typée qui n'est pas accessible dans les arguments d'outil fournis par l'agent.
