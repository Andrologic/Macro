...
Dans un processus Macro, chaque mutation acquiert un verrou associé à la cible canonique avant de valider la révision et le conserve jusqu'à la fin de l'écriture, de la suppression ou du rollback. Les lots multi-fichiers trient et dédupliquent leurs verrous avant acquisition afin d'éviter les interblocages. Les écritures utilisent ensuite un fichier temporaire et un renommage atomique, avec une dernière revalidation au plus près du renommage. Cette exclusion ne s'étend pas aux applications externes qui n'utilisent pas le protocole : elles peuvent encore gagner une course dans l'intervalle résiduel. Le pont Copilot calcule les mêmes révisions sur ses lectures locales. Un transport distant doit annoncer la capacité `content_revisions_v1` avant d'accepter une mutation gardée.
