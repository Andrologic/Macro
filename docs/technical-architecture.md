...
Un projet `not_git` peut être marqué `directEdit`. Il est alors modifiable dans Implement et Architect. Les plans enregistrent `executionModesByProjectId` sur leurs nœuds. Un plan mixte provisionne les branches et les worktrees de ses seules cibles Git. Les cibles directes s'exécutent dans le chemin du projet, sans nom de branche ni worktree, et le filtre de capacités retire les outils Git lorsqu'elles sont focalisées. L'exécuteur vérifie aussi le mode de la cible avant tout appel backend, ce qui protège les plans mixtes même lorsque leur catalogue global contient des outils Git.
