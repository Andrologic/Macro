...
Macro is an open-source, local-first desktop environment for agentic software work. Use this
...
- Treat all tracked content as public, including code, documentation, agent
  instructions, examples, fixtures, and commit messages. Keep personal data,
  private conversations, customer information, internal business plans, and
  machine-specific paths or configuration outside the repository. Use synthetic
  examples and portable paths. Before adding third-party code or assets, check
  that their license permits inclusion and preserve required attribution.
...
- Treat GitHub Actions as the exhaustive merge gate, never as a development
  loop. Before pushing, opening a ready pull request, or updating an existing
  pull request, run the fast changed-file gate with `bun run ci:pre-push` and
  fix failures locally. Run `bun run ci` before releases and when a full local
  rehearsal is explicitly needed.
- Never bypass repository hooks with `--no-verify` unless the user explicitly
  ...
  locally and pass the relevant focused check or full local profile before
  pushing another fix.
...
- Fast differential pre-push gate: `bun run ci:pre-push`.
